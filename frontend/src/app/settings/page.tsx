    ...
    <div className="flex min-h-screen bg-[#F5F0EB]">
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="p-8 max-w-4xl">
          <h1 className="text-3xl font-bold tracking-tight mb-1 text-[#2C2420]">Settings</h1>
          <p className="text-sm mb-8 text-[#A19B95]">AIRA OS system information &amp; connection</p>
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              <Server className="w-4 h-4 text-[#8B5A2B]" />
              <h2 className="font-bold text-sm text-[#2C2420]">API Connection</h2>
            </div>
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-[#EDE5DC] border border-zinc-100">
                  <div className="flex items-center gap-2">
                    <item.icon className="w-3.5 h-3.5 text-[#A19B95]" />
                    <span className="text-sm text-[#716B65]">{item.label}</span>
                  </div>
                  {item.isLink ? (
                    <a href={item.linkUrl} target="_blank" rel="noopener noreferrer"
                       className="text-xs text-primary hover:underline">{item.value}</a>
                  ) : item.mono ? (
                    <code className="text-xs font-mono text-[#5A544E] bg-zinc-100 px-1.5 py-0.5 rounded">{item.value}</code>
                  ) : (
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="font-bold text-sm text-[#2C2420]">About AIRA OS</h2>
            </div>
            <p className="text-sm leading-relaxed mb-2 text-[#716B65]">
            ...
            <p className="text-xs italic text-[#A19B95]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {PLANETS.map((p) => (
                <Link key={p.id} href={`/planets/${p.id}`}
                  className="flex items-center gap-3 p-3 rounded-xl hover:bg-[#EDE5DC] transition-all bg-white border border-zinc-100"
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[11px] truncate text-[#A19B95]">{p.role}</p>
                  </div>
                </Link>
              ))}
            </div>
